...
function GroupMembersRecents({ form }: { form: GroupMembersFormReturn }) {
	...
	const handleSetLoaded = () => setIsContactsLoaded(true);
	...
	return (
		...
		>
			...
			<div className="flex flex-col pt-2">
				{isContactsLoaded && <RecentGroupResults search={search} form={form} />}
...
function RecentContactsResults({ search, form, handleSetLoaded }: RecentResultsProps) {
	...
	const { data, isSuccess } = api.contact.getRecentContacts.useQuery({
		search,
		addedContactIds,
	});

	useEffect(() => {
		if (isSuccess) handleSetLoaded();
	}, [isSuccess]);
